	/*
		Программа включает и выключает светодиод по кнопке нажатию и отпусканию соответственно. 
	*/
	#include "stm32f4xx.h"

	volatile uint8_t button_pressed = 0;
	volatile uint32_t button_released = 0;
	...
	void TIM2_IRQHandler(void){
		...
		else if(button_released == 1){
			button_released = 0;
			if ((GPIOA->IDR & GPIO_IDR_ID0)) {
				// Кнопка все еще отпущена - это реальное событие
				GPIOC->ODR ^= GPIO_ODR_OD13; 
			}
		}
