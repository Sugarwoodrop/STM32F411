/*
  Программа включает и выключает светодиод по кнопке. Используется прерывания по нажатию, а таймер с прерыванием используется для анти дребезга 
*/
#include "stm32f4xx.h"
...
void GPIOC_Init (void) {
	...
	EXTI->FTSR|= EXTI_FTSR_TR0; //Прерывание срабатывает на спадющий фронт (кнопка замкнута на GND).
